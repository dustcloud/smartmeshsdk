...
ERR_INVALID             = ERR_BASE+2

#===== application

WINDOW_TITLE = 'DC2126A receiver'
RESPONSE_TIMEOUT = 10000 # in ms
...
class dc2126aGui(object):
    ...
    def __init__(self):
        ...
        # Flag
        self.respReceivedFlag = False
        self.isTimeoutRunning = False
    ...
    def _getConfigurationCB(self):
        ...
        if not self.isTimeoutRunning:
            ...
            self.window.after(RESPONSE_TIMEOUT, 
                              self._displayNoConfigurationReceviedError)
            self.isTimeoutRunning = True
    ...
    def _setConfigurationCB(self,reportPeriod,bridgeSettlingTime,ldoOnTime):
        ...
        # update action
        self.configurationFrame.writeActionMsg('set configuration')
    
    def _displayConfiguration(self,configuration):
        
        # Stop response timeout
        self.respReceivedFlag = True
    ...
    def _displayNoConfigurationReceviedError(self):
        self.isTimeoutRunning = False
        if not self.respReceivedFlag:
            self._displayError('No mote configuration received')
            self.configurationFrame.writeActionMsg('')
