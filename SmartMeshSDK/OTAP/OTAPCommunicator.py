...
from ReliableCommander import ReliableCommander
...
class OTAPCommunicator(object):
    ...
    def __init__(self, send_data, notif_listener, motes = None, auto_commit = True):
        ...
        self.orc = ReliableCommander(self.send_data, self.cmd_failure_callback)
    ...
    def wait_for_data_complete(self):
        ...
        while not self.data_event.is_set():
            self.data_event.wait(WAIT_TIMEOUT)

    def wait_for_commit_complete(self):
        # wait has a timeout so that we catch user interruptions
        while not self.commit_event.is_set():
            self.commit_event.wait(WAIT_TIMEOUT)
    ...
    def data_callback(self, data):
        ...
        if data.src_port == OTAP_PORT:
            ...
            while index < len(data.payload):
                ...
                self.orc.received(data.mac, cmd_type)
    ...
    def handshake_callback(self, mac, cmd_data):
        ...
        # remove this mote from the list of expected handshakes
        
        self.handshake_motes.remove(mac)
        # once the list of expected handshakes is empty, we're ready to move on
    ...
    def data_task(self):
        ...
        try:
            for bnum in blist:
                ...
                if deps <= BROADCAST_THRESHOLD and len(macs):
                    for m in macs:
                ...
                # TODO: wait for inter-data delay
    ...
    def send_otap_cmd(self, mac, cmd_id, data):
        ...
        while rc != 0 and count < DATA_RETRIES:
            if rc == END_OF_LIST:
    ...
    def send_reliable_cmd(self, mac, cmd_id, data):
        return self.orc.send(mac, OTAP_PORT, cmd_id, data)
